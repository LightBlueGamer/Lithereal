...
public class BlockStateBaseMixin {
    ...
    public float newDestroySpeed(BlockGetter blockGetter, BlockPos blockPos, Operation<Float> original) {
        if (!(blockGetter instanceof Level level) || !level.dimension().identifier().equals(Lithereal.id("ethereal_core")) || !EtherealCoreArenaFeature.UNCHANGEABLE.isInside(blockPos)) return original.call(blockGetter, blockPos);
        return -1;
    }
}