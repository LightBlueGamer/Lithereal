...
public class CapFoliagePlacer extends FoliagePlacer {
	public static final MapCodec<CapFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(
		instance -> foliagePlacerParts(instance).and(IntProviders.POSITIVE_CODEC.fieldOf("max_height").forGetter(CapFoliagePlacer::maxHeight)).apply(instance, CapFoliagePlacer::new)
	);
	...
	protected void createFoliage(
		...
		for (int yOff = -(foliageHeight - 1); yOff <= 0; yOff++) {
			...
			if (yOff == 0)
				foliageRadiusOffset += 1;
			this.placeLeavesRow(worldGenLevel, foliageSetter, randomSource, treeConfiguration, blockPos, foliageRadius + foliageAttachment.radiusOffset() - foliageRadiusOffset, yOff, doubleTrunk);
		}
	}
