		...
		if (!DripstoneReplacementUtils.isEmptyOrWater(worldGenLevel, blockPos)) {
			...
			if (optional.isPresent() && optional.get() instanceof Column.Range range) {
                if (range.height() < 4) {
					...
					int j = Mth.clamp(i, largeSpikeConfiguration.columnRadius().minInclusive(), largeSpikeConfiguration.columnRadius().maxInclusive());
					int k = Mth.randomBetweenInclusive(randomSource, largeSpikeConfiguration.columnRadius().minInclusive(), j);
