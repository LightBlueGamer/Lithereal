...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        blockWithItem(ModOreBlocks.PAILITE_SATURNITE_ORE);
        blockWithItem(ModOreBlocks.HELLIONITE_ORE);
        blockWithItem(ModOreBlocks.PAILITE_HELLIONITE_ORE);
        blockWithItem(ModOreBlocks.ELUNITE_ORE);
        blockWithItem(ModOreBlocks.CHRYON_ORE);
        blockWithItem(ModOreBlocks.ALLIAN_ORE);
        blockWithItem(ModOreBlocks.PAILITE_NETHERITE_ORE);

        blockWithItem(ModPhantomBlocks.PHANTOM_DIAMOND_ORE);
        blockWithItem(ModPhantomBlocks.PHANTOM_QUARTZ_ORE);
    ...
    private void leavesBlock(Block block, Identifier texture) {
        simpleBlock(block, models().leaves(name(block), texture));
    }

    private void cubeColumn(Block block, Identifier side, Identifier end) {
    ...
    private void plant(Block normal, Block potted, TintState tintState, Identifier texture) {
        simpleBlock(normal, models().singleTexture(name(normal), Identifier.parse(tintState.getCross()), tintState.getCrossTexture(), texture));
        simpleBlock(potted, models().singleTexture(name(potted), Identifier.parse(tintState.getCrossPot()), tintState.getPotTexture(), texture));
    }

    private void plantWithItem(Block normal, Block potted, TintState tintState) {
        Identifier texture = modLoc("block/" + name(normal));
